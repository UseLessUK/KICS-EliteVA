...
If you already have the **Ripley Galactic KICS** profile audio files installed then all you need to download and import, into VoiceAttack, are the .**vap** files, you will need both the `Common.commands-Profile.vap` along with the `Ripley Galactic KICS 4.1.2 [B 0.32] [EliteVA]-Profile.vap` files.
...
If you're updating from a previous build then simply **delete** the `common.commands` profile and then import the 2 updated profiles, you can also if you wish remove the previous build profile;
	...
	- Ripley Galactic KICS 4.1.2 [B 0.32] [EliteVA]-Profile.vap
